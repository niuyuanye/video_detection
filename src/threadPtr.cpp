 ...
 * @LastEditTime: 2023-02-24 08:51:44
 * @FilePath: /video_detection/src/threadPtr.cpp
...
// int ThreadPtr::a = 0;
ThreadPtr::ThreadPtr() : autoDelete_(false) // 默认是false
{
    std::cout << "ThreadPtr Constructor" << std::endl;
}
ThreadPtr::~ThreadPtr()
{
    std::cout << "ThreadPtr Destructor" << std::endl;
}

void ThreadPtr::Start()
{
...
void *ThreadPtr::ThreadPtr_routine(void *arg) // 类的静态函数
{
    ...
    // 基类指针，指向派生类对象
    ThreadPtr *thread = static_cast<ThreadPtr *>(arg); // arg是this指针，作为参数传递进来，将派生类指针转换为基类指针
...
void ThreadPtr::SetAutoDelete(bool arg)
{
    autoDelete_ = arg;
}
