...
void YOLOv8Seg::showAndSave(const std::vector<std::string>& classNames, const int& cvDelayTime, std::vector<cv::Mat>& imgsBatch)
{
    ...
    for (size_t bi = 0; bi < imgsBatch.size(); bi++)
    {
        ...
        Eigen::Map<Eigen::MatrixXf> img_seg_(p_seg, m_output_seg_w, m_output_seg_h);//使用eigen的map功能进行内存映射 [160*160,25]
        ...
        for (size_t i = 0; i < num_boxes; i++)
        {
            ...
            if (ptr[6])
            {
                ...
                Eigen::Map<Eigen::MatrixXf> img_ojb_seg_(ptr + 7, m_output_seg_h, 1); //使用eigen的map功能进行内存映射 [25,1]
                m_mask_eigen160 = img_seg_ * img_ojb_seg_;//矩阵相乘 [160*160,25]*[25,1]=[160*160,1]
                cv::eigen2cv(m_mask_eigen160, m_mask160);
                cv::exp(-m_mask160, m_mask160); 
                m_mask160 = 1.f / (1.f + m_mask160);//用opencv做sigmoid
                ...
                cv::Mat mask_instance;
                cv::resize(cv::Mat(m_mask160, roi160), mask_instance, cv::Size(roisrc.width, roisrc.height), cv::INTER_LINEAR);
                mask_instance = mask_instance > 0.5f;//表示此像素是前景障碍物
        ...
        {
            cv::Mat img ;
            cv::resize(imgsBatch[bi] + m_img_canvas,img,cv::Size(640,480)); 
            // cv::imshow(m_param.winname, imgsBatch[bi] + m_img_canvas);
            // cv::imshow("fy-yolov8-seg", m_img_canvas);
            cv::imshow("fy-yolov8-seg", img);
