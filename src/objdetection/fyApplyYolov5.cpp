 ...
 * @LastEditTime: 2023-03-02 00:52:40
 * @LastEditors: nyy
 * @Description: 
 * @FilePath: /video_detection/src/objdetection/fyApplyYolov5.cpp
...
fyApplyYolov5::fyApplyYolov5(bool stop = false) : stop_(stop){
    LOG_INFO("\n ProApply start \n ");
    pthread_mutex_init(&this->lock_p,NULL);

}
fyApplyYolov5::~fyApplyYolov5(){
    LOG_INFO("\n ProApply end \n ");
}

void fyApplyYolov5::Stop(){
    this->stop_ = true;    
}
...
int fyApplyYolov5::iniModel(){
    ...
    if (n >= 0) LOG_INFO("yolov5_version_tmp"<<":"<<yolov5_version_tmp);
	...
	*************************************************************************************************/
	...
	LOG_INFO("yolov5_version"<<":"<<yolov5_version);
	...
	// YOLOV5 yolo(param);
    yolo = std::make_shared<YOLOV5>(param);
 ...
 * @return {*}
 */
void fyApplyYolov5::Run() // 虚函数的重写可以改变成员函数的权限
{    
    bool is_show = true;
	bool is_save = true;
    int delay_time = 1;
    ...
    {       
        ...
        if(camid==target_id){
            LOG_INFO("FY yolov5 time:"<<time_out<<" ms "<<camid); 
 ...
 * @return {*}
 */
void fyApplyYolov5::eventFunc(cv::Mat img, std::string img_path,std::string image_time,int camid)
{
    auto start = std::chrono::system_clock::now();  
 ...
 * @return {*}
 */
void fyApplyYolov5::setParameters(utils::InitParameter& initParameters)
{
