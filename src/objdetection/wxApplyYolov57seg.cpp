...
wxApplyYolov57seg::wxApplyYolov57seg(bool stop = false) : stop_(stop){
    LOG_INFO("\n wxApplyYolov56 start \n ");
    pthread_mutex_init(&this->lock_p,NULL);
}
wxApplyYolov57seg::~wxApplyYolov57seg(){
    LOG_INFO("\n wxApplyYolov57seg end \n ");
}

void wxApplyYolov57seg::Stop(){
    this->stop_ = true;    
}
 ...
 * @return {*}
 */
int wxApplyYolov57seg::iniModel(){  
   	/************************************************************************************************
	* init
	*************************************************************************************************/
    ...
    char enginename[100];
    long n = ini_gets("WX", "engine_seg_name", "../weigths/yolov57n-seg.engine", enginename, sizearray(enginename), const_cast<char *>(inifile.c_str()));
    if (n >= 0) LOG_INFO("engine_seg_name"<<":"<<enginename);
	...
	*************************************************************************************************/
    ...
    yolo = std::make_shared<Yolov5_v7segTRT>();
    return yolo->InitialSeg(enginename,NMS_THRESH,CONF_THRESH); 
 ...
 * @return {*}
 */
void wxApplyYolov57seg::Run() // 虚函数的重写可以改变成员函数的权限
{    
    bool is_show = true;
	bool is_save = true;
    int delay_time = 1;
    ...
    {       
        ...
        if(camid==target_id){
            LOG_INFO("WX yolov57seg time:"<<time_out<<" ms "<<camid); 
 ...
 * @return {*}
 */
void wxApplyYolov57seg::eventFunc(cv::Mat img, std::string img_path,std::string image_time,int camid)
{
    auto start = std::chrono::system_clock::now();  
...
cv::Rect wxApplyYolov57seg::Get_rect(cv::Mat& img, float bbox[4]) {
