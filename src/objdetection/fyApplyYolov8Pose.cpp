 ...
 * @LastEditors: nyy
 * @Description: 
 * @FilePath: /irs-orin/src/objdetection/fyApplyYolov8Pose.cpp
...
fyApplyYolov8Pose::fyApplyYolov8Pose(bool stop = false) : stop_(stop){
    LOG_INFO("\n fyApplyYolov8Pose start \n ");
    pthread_mutex_init(&this->lock_p,NULL);
}
fyApplyYolov8Pose::~fyApplyYolov8Pose(){
    LOG_INFO("\n fyApplyYolov8Pose end \n ");
}

void fyApplyYolov8Pose::Stop(){
    this->stop_ = true;    
}
 ...
 * @return {*}
 */
int fyApplyYolov8Pose::iniModel(){
    ...
    long n = ini_gets("FY", "yolov8model_pose", "../weigths/yolov8s.trt", engineModel, sizearray(engineModel), const_cast<char *>(inifile.c_str()));
	...
	*************************************************************************************************/
    ...
    yoloPose = std::make_shared<YOLOv8Pose>(param);
...
int fyApplyYolov8Pose::yolov8PoseDect(cv::Mat image){
    ...
    yoloPose->copy(imgs_batch);
    utils::DeviceTimer d_t1; yoloPose->preprocess(imgs_batch);  float t1 = d_t1.getUsedTime();
    utils::DeviceTimer d_t2; yoloPose->infer();				  float t2 = d_t2.getUsedTime();
    utils::DeviceTimer d_t3; yoloPose->postprocess(imgs_batch); float t3 = d_t3.getUsedTime();
    ...
    // yoloPose->showAndSave(param.class_names, delay_time, imgs_batch);
    yoloPose->showAndSave(param.class_names, delay_time, imgs_batch, avg_times);
 ...
 * @return {*}
 */
void fyApplyYolov8Pose::Run() // 虚函数的重写可以改变成员函数的权限
{    
    bool is_show = true;
	bool is_save = true;
    int delay_time = 1;
    ...
    {       
        ...
        // LOG_INFO("yolov8-pose time:"<< time_out);
        // start =  end;
 ...
 * @return {*}
 */
void fyApplyYolov8Pose::setParameters(utils::InitParameter& initParameters)
{
 ...
 * @return {*}
 */
void fyApplyYolov8Pose::eventFunc(cv::Mat img, std::string img_path,std::string image_time,int camid)
{
    auto start = std::chrono::system_clock::now();  
